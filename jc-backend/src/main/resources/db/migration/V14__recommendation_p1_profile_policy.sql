-- Journey Connect P2 Flyway port - P1 profile, policy assignment, and comparison evidence
-- Ensure P1 vocabulary tags exist in the target tag schema.
INSERT INTO public.tag (name, normalized_name)
VALUES
  ('solo-travel', 'solo-travel'),
  ('couple-trip', 'couple-trip'),
...
CREATE TABLE public.recommendation_p1_profile_snapshot (
  ...
  user_id bigint NOT NULL REFERENCES public.user_account(id),
...
BEGIN
  ...
  IF v_user_id IS NULL OR v_user_id <= 0 OR NOT EXISTS (
    ...
    RAISE EXCEPTION 'Recommendation preference user must reference an active account.'
      USING ERRCODE = '42501';
  END IF;
