-- Journey Connect P2 Flyway port - behavior runtime and atomic post interactions
...
BEGIN
  ...
  IF v_user_id IS NULL OR v_user_id <= 0 OR NOT EXISTS (
    SELECT 1 FROM public.user_account u
    WHERE u.id = v_user_id AND u.account_status = 'active'
  ) THEN
    RAISE EXCEPTION 'Interaction user must reference an active account.'
      USING ERRCODE = '42501';
  END IF;
  ...
  IF p_action IN ('like', 'save') AND NOT EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION 'Post % is not visible or published.', p_post_id
      USING ERRCODE = '42501';
  END IF;
  ...
  ELSIF p_action = 'save' THEN
    INSERT INTO public.bookmark (post_id, user_id)
    VALUES (p_post_id, v_user_id)
    ON CONFLICT DO NOTHING;
    GET DIAGNOSTICS v_changed = ROW_COUNT;
  ELSE
