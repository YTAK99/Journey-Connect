-- Journey Connect P2 Flyway port - P0 recommendation storage
-- Target baseline: existing Flyway V1..V10 (user_account/journey_post/region schema).
-- Ported from canonical DB v2.7 SQL 07 with the SQL 17 partition correction folded in.
...
BEGIN
  ...
  FROM (
    VALUES
      ('public.user_account', to_regclass('public.user_account')),
      ('public.journey_post', to_regclass('public.journey_post')),
      ('public.region', to_regclass('public.region')),
      ('public.post_like', to_regclass('public.post_like')),
      ('public.bookmark', to_regclass('public.bookmark'))
  ) AS required(required_object, object_oid)
  WHERE object_oid IS NULL;

  IF v_missing IS NOT NULL THEN
    RAISE EXCEPTION 'Flyway V1..V10 prerequisite objects are missing: %', v_missing
      USING ERRCODE = '42P01';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'journey_post'
      AND column_name = 'moderation_status'
  ) THEN
    RAISE EXCEPTION 'Flyway V1..V10 prerequisite column public.journey_post.moderation_status is missing.'
      USING ERRCODE = '42703';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Flyway V1..V10 prerequisite column public.user_account.account_status is missing.'
      USING ERRCODE = '42703';
  END IF;
END;
$$;
...
CREATE TABLE public.recommendation_run (
  ...
  user_id bigint NOT NULL REFERENCES public.user_account(id) ON DELETE RESTRICT,
...
BEGIN
  IF NOT EXISTS (
    ...
    JOIN public.journey_post p ON p.id = NEW.source_entity_id
    JOIN public.user_account author_user ON author_user.id = p.author_id
...
CREATE TABLE public.recommendation_behavior_event (
  ...
  user_id bigint REFERENCES public.user_account(id) ON DELETE RESTRICT,
  session_id varchar(128) NOT NULL,
