...
public class RecommendationCandidateSource {
    ...
    private static final String FIND_ELIGIBLE_BY_REGION = """
            select p.id,
                   p.author_id,
                   lower(r.code) as region_slug,
                   'public' as visibility,
    ...
    public List<RecommendationCandidateRow> findEligible(
            long viewerId, String regionCode, int limit) {
        if (regionCode == null || regionCode.isBlank()) {
            return findEligible(viewerId, limit);
        }
        if (viewerId <= 0) {
            throw new IllegalArgumentException("viewerId must be positive");
        }
