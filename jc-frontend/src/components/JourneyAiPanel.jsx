...
function JourneyAiPanel() {
  ...
  useEffect(() => {
    ...
    const closeOnOutsidePointer = (event) => {
      ...
      if (!clickedPanel && !clickedToggle) setOpen(false);
    };
  ...
  return (
    <>
      <button
        ...
        className={`fixed bottom-6 inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-teal-600 to-cyan-600 text-sm font-bold text-white shadow-[0_16px_40px_-12px_rgba(13,148,136,0.75)] transition hover:-translate-y-0.5 hover:shadow-xl ${
          open
            ? "left-5 z-[90] h-11 w-11 p-0 sm:left-auto sm:right-[454px]"
            : "right-5 z-[70] px-5 py-3 sm:right-8"
        }`}
        aria-label={copy.title}
        aria-expanded={open}
      >
        {open ? <X size={19} /> : <><Sparkles size={18} />{copy.title}</>}
      </button>

      {open && (
        <div className="fixed inset-0 z-[80] bg-slate-950/20 backdrop-blur-[1px] sm:bg-transparent sm:backdrop-blur-none">
          <section ref={panelRef} className="absolute inset-y-0 right-0 flex w-full flex-col border-l border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950 sm:w-[430px]">
