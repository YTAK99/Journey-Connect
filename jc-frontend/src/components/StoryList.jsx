    ...
    <div className="w-full py-2">
      <div className="flex items-center gap-6 overflow-x-auto py-2">
        <button type="button" className="group flex shrink-0 cursor-pointer flex-col items-center">
          <span className="flex h-16 w-16 items-center justify-center rounded-full border-2 border-dashed border-teal-400 bg-teal-50 transition group-hover:bg-teal-100">
            <Plus size={24} className="text-teal-500" />
          </span>
          <span className="mt-2 text-xs text-gray-600">{currentLang === "ko" ? "올리기" : "Post"}</span>
        </button>

        {stories.map((story) => (
          <button key={story.id} type="button" className="group flex shrink-0 cursor-pointer flex-col items-center">
            <span className="h-16 w-16 overflow-hidden rounded-full border-2 border-teal-500 p-0.5">
              <img
                src={story.image}
                alt={story.region}
            ...
            <span className="mt-2 max-w-[70px] truncate text-center text-xs text-gray-700">{story.region}</span>
          </button>
        ))}
      </div>
    </div>
