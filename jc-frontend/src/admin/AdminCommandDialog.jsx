...
export default function AdminCommandDialog({ open, title, description, confirmLabel, pending, onClose, onConfirm, confirmationLabel, expectedConfirmation }) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event) => {
      ...
      const controls = dialogRef.current?.querySelectorAll('button:not([disabled]), textarea:not([disabled]), input:not([disabled])');
  ...
  const submit = async (event) => {
    ...
    if (expectedConfirmation && confirmation.trim() !== String(expectedConfirmation)) { setError("확인 값이 일치하지 않습니다."); return; }
    if (pending) return;
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4" onMouseDown={(event) => event.target === event.currentTarget && !pending && onClose()}>
    <form ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="admin-command-title" aria-describedby="admin-command-description" onSubmit={submit} className="w-full max-w-lg rounded-2xl bg-white shadow-2xl">
      ...
      <div className="space-y-5 p-6"><div><label htmlFor="admin-command-reason" className="mb-2 block text-sm font-semibold text-slate-700">처리 사유</label><textarea ref={textareaRef} id="admin-command-reason" required maxLength={ADMIN_MAX_REASON_LENGTH} rows="5" value={reason} aria-describedby="admin-command-reason-help admin-command-reason-error" aria-invalid={Boolean(error)} onChange={(event) => { setReason(event.target.value); setError(""); }} className="w-full resize-y rounded-lg border border-slate-300 px-3 py-2.5 outline-none focus:border-teal-600 focus:ring-2 focus:ring-teal-100" /><div id="admin-command-reason-help" className="mt-1 flex justify-between text-xs text-slate-500"><span>필수 입력 · 최대 1000자</span><span>{reason.length}/1000</span></div></div>{expectedConfirmation && <div><label htmlFor="admin-command-confirmation" className="mb-2 block text-sm font-semibold text-rose-800">{confirmationLabel || "확인 값"}</label><input id="admin-command-confirmation" value={confirmation} autoComplete="off" onChange={(event) => { setConfirmation(event.target.value); setError(""); }} className="w-full rounded-lg border border-rose-300 px-3 py-2.5 outline-none focus:border-rose-600 focus:ring-2 focus:ring-rose-100" /></div>}{error && <p id="admin-command-reason-error" role="alert" className="text-sm text-red-700">{error}</p>}</div>
      <div className="flex justify-end gap-3 border-t border-slate-200 bg-slate-50 px-6 py-4"><button type="button" disabled={pending} onClick={onClose} className="rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold disabled:opacity-40">취소</button><button type="submit" disabled={pending || !reason.trim() || (expectedConfirmation && confirmation.trim() !== String(expectedConfirmation))} className={`rounded-lg px-4 py-2.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50 ${expectedConfirmation ? "bg-rose-700 hover:bg-rose-800" : "bg-teal-700 hover:bg-teal-800"}`}>{pending ? "처리 중..." : confirmLabel}</button></div>
    </form>
  </div>;
}
