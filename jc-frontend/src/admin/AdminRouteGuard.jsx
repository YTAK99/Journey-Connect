...
export default function AdminRouteGuard({ children }) {
  ...
  if (state.status === "forbidden") return <main className="grid min-h-screen place-items-center bg-slate-50 p-6"><div className="max-w-md rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm"><h1 className="text-xl font-bold text-slate-950">관리자 접근 불가</h1><p className="mt-3 text-sm text-slate-600">관리자 권한이 없습니다.</p><button type="button" onClick={() => { clearStoredAuth(); navigate("/admin/login", { replace: true }); }} className="mt-5 inline-flex rounded-lg bg-teal-700 px-4 py-2.5 text-sm font-semibold text-white">다른 계정으로 로그인</button></div></main>;
